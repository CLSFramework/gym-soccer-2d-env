...
parser.add_argument(
    "--useturn", action="store_true", help="Enable turning", default=True
)
parser.add_argument(
    "--actor_out_size", type=int, default=4, help="Size of the actor output"
)
...
def train(
    ...
    if env.continuous:
        model = DDPG("MlpPolicy", env, verbose=1, tensorboard_log=log_dir, policy_kwargs=policy_kwargs)
    else:
        model = DQN(
            "MlpPolicy",
            env,
...
def ddpg_objective(trial):
    ...
    actor_layer_size = trial.suggest_categorical("actor_layer_size", [8, 16, 32, 64, 128, 256, 400])
    critic_layer_size = trial.suggest_categorical("critic_layer_size", [8, 16, 32, 64, 128, 256, 400])
    actor_n_layers = trial.suggest_int("actor_n_layers", 1, 5)
    critic_n_layers = trial.suggest_int("critic_n_layers", 1, 5)
    ...
    # )
    actor_net_arch = [actor_layer_size] * actor_n_layers
    critic_net_arch = [critic_layer_size] * critic_n_layers
    policy_kwargs = dict(
        net_arch={
            'pi': actor_net_arch,
            'qf': critic_net_arch,
...
study.optimize(ddpg_objective, n_trials=50, callbacks=[logging_callback])
